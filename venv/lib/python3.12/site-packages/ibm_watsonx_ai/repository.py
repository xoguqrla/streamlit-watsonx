...
from ibm_watsonx_ai.utils.utils import _get_id_from_deprecated_uid, _is_iam_token
...
class Repository(WMLResource):
    ...
    def _refresh_repo_client(self) -> None:
        ...
        if self._client.proceed is True:
            # _is_iam_token is used here in strange way
            # it is checked if token is iam and has instance_id or is not iam token
            if _is_iam_token(self._client.token) is not None:
            ...
            if self._client._is_IAM():
                ...
                if self._client.ICP_PLATFORM_SPACES:
                    self._repotoken = self._client.token
